...
public class Pixel implements Comparable<Pixel> {
    ...
    // Creating an array to store all three different colors of pixel
    private int[] pixelColors = new int[3];
    ...
    public int distance(Pixel px) {
        return Math.abs(pixelColors[0] - px.getRed()) +
                Math.abs(pixelColors[1] - px.getGreen()) +
                Math.abs(pixelColors[2] - px.getBlue());
    }
    ...
    public boolean equals(Pixel px) {
        return pixelColors.equals(px.pixelColors);
    }
