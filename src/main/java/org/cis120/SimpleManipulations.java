...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        // adjusted width and height for our new bitmap tgt
        int adW = w + 2 * borderWidth;
        int adH = h + 2 * borderWidth;

        Pixel[][] src = pic.getBitmap();
        // our custom 2-D array that will be filled with pixels
        Pixel[][] tgt = new Pixel[adH][adW];

        for (int y = 0; y < adH; y++) {
            for (int x = 0; x < adW; x++) {

                // draw top and bottom border
                if (y < borderWidth || y > h + borderWidth - 1) {
                    tgt[y][x] = borderColor;

                    // draw left and right border
                } else if (x < borderWidth || x > w + borderWidth - 1) {
                    tgt[y][x] = borderColor;
                } else {
                    tgt[y][x] = src[y - borderWidth][x - borderWidth]; // draw the real picture
                }
    ...
    public static int weightedAverage(double alpha, int x, int y) {
        if (alpha == 0.0) {
            ...
            double firstHalf = x * alpha;
            double secondHalf = y * (1 - alpha);
            return (int) Math.round(firstHalf + secondHalf);
        }
    }
    ...
    public static PixelPicture alphaBlend(
        ...
        if (w == w2 && h == h2) {
            Pixel[][] tgt1 = pic.getBitmap();
            Pixel[][] tgt2 = f.getBitmap();
            for (int col = 0; col < w; col++) {
                for (int row = 0; row < h; row++) {
                    Pixel p1 = tgt1[row][col];
                    Pixel p2 = tgt2[row][col];
                    ...
                    tgt1[row][col] = new Pixel(r, g, b);

                } // end inner loop
            } // end outer loop
            return new PixelPicture(tgt1);
        } else {
            return pic;
        }
    }
